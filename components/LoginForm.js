...
export default function LoginForm() {
  ...
  const handleLogin = async (event) => {
    ...
    try{
      const response = await fetch('../pages/api/login', { username: email, password });
    ...
    return (
      ...
      <form onSubmit={handleLogin} className="p-10 bg-black rounded-xl drop-shadow-lg space-y-5" action="">
        ...
        <div className="flex flex-col space-y-2">
          ...
          <input
            ...
            type="email" placeholder="Your Email" name="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)}
          />
        </div>
        ...
        <div className="flex flex-col space-y-2">
          ...
          <input
            className="w-96 px-3 py-2 rounded-md border border-slate-400"
            type="password" placeholder="Your Password" name="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)}
          />
        </div>
